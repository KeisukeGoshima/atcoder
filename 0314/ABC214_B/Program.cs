...
namespace ABC
{
    class Program
    {
        ...
        void solve()
        {
			...
			var S = input[0];
			var T = input[1];
			var total = Enumerable.Range(0, S+1).Select(x => {
				var tmp = Enumerable.Range(0, S+1-x).Select(y => {
					var tmp2 = Enumerable.Range(0, S+1-x-y).Select(z => {
						if (x * y * z <= T) return 1;
						else return 0;
					}).ToList();
					return tmp2;
			...
			var total2 = total.SelectMany(x => x).ToList().SelectMany(x => x).ToList();
			Console.WriteLine(total2.Count(x => x == 1));
        }
