...
namespace ABC
{
    class Program
    {
        ...
        void solve()
        {
			...
			input.Reverse();
			var index = input.FindIndex(x => x != '0');
			if (index == -1) Console.WriteLine("Yes");
			else {
				var result = Enumerable.Range(index, input.Count - index).Select(x => input[x] == input[input.Count - 1 + index - x]).ToList();
				if (result.All(x => x == true)) Console.WriteLine("Yes");
