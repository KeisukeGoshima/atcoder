...
namespace ABC
{
    class Program
    {
        ...
        void solve()
        {
			...
			var resultIndex = Enumerable.Range(0, N).Where(x => C[x] == K).Select(x => x).ToList();
			var result = resultIndex.Select(x => R[x]).ToList();
            var maxi = result.Max();
			var maxIndex = result.FindIndex(x => x == maxi);
			Console.WriteLine(resultIndex[maxIndex]+1);
        }
