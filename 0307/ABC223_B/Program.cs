...
namespace ABC
{
    class Program
    {
        ...
        void solve()
        {
            var input = ReadStringList();
            if (input == null) return ;
			var S = input[0].ToList();
			var result = Enumerable.Range(0, S.Count).Select(x => {
				var tmp = new List<char>();
				var _ = Enumerable.Range(x, S.Count).Select(y => tmp.Add(S[y])).ToList();
				_ = Enumerable.Range(0, x).Select(y => tmp.Add(S[y])).ToList();
				return tmp.ToString();
			}).ToList();
