using System;
using System.Linq;
using System.Collections.Generic;

namespace ABC
{
    class Program
    {
        static void Main(string[] args)
        {
            var program = new Program();
            program.solve();
        }

        void solve()
        {
            var input = Console.ReadLine().Split(" ");
            if (input == null) return ;
			var A = input[0];
			var A_int = A.Select(x => {
				var tmp = x - '0';
				return tmp;
			}).ToList();
			var B = input[1];
			var B_int = B.Select(x => {
				var tmp = x - '0';
				return tmp;
			}).ToList();
			A_int.Reverse();
			B_int.Reverse();
			var length = Math.Min(A.Count(), B.Count());
			var result = Enumerable.Range(0, length).Select(x => A_int[x] + B_int[x] < 10).ToList();
			if (result.Any(x => x == false)) Console.WriteLine("Hard");
			else Console.WriteLine("Easy");

        }

        List<string> ReadStringList(int N=1)
        {
            var input = Enumerable.Range(0, N).Select(_ => Console.ReadLine()).ToList();
            return input;
        }
        List<int> ReadInt()
        {
            var check = new List<bool>();
            var input = Console.ReadLine().Split(' ').Select(x => {
                var temp = 0;
                check.Add(int.TryParse(x, out temp));
                return temp;
            }).ToList();
            if (check.Any(x => x == false)) return null;
            return input;
        }
        void WriteInt(int output, bool nextline=true)
        {
			if (nextline==true)
	            Console.WriteLine(output);
			else
				Console.Write("{0} ", output);
        }
        void WriteLong(long output, bool nextline=true)
        {
			if (nextline==true)
	            Console.WriteLine(output);
			else
				Console.Write("{0} ", output);
        }
        void WriteIntList(List<int> output)
        {
            output.ForEach(x => Console.Write("{0} ", x));
            Console.WriteLine("");
        }
    }
}